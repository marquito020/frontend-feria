...
import { useGetInverters } from "../../hooks/useInverter.hook";
...
function Inverters() {
  ...
  const { inverters, isLoading, error } = useGetInverters();

  console.log("inverteres", Inverters);
              ...
              <div key={index}>
                ...
                <div className="mt-4 flex justify-between">
                  <div>
                    ...
                    <p className="text-sm font-medium text-gray-900">
                      Potencia Nominal : {inverter.potenciaNominalW} W
                    </p>
                    <p className="text-sm font-medium text-gray-900">
                      Potenica Pico : {inverter.potenciaPicoW} W
                    </p>
                    <p className="text-sm font-medium text-gray-900">
                      Caracteristicas: {inverter.caracteristicas}
                    </p>
                    {/* Precio */}
                    <p className="mt-2 text-sm font-medium text-gray-900">
                      Precio: {inverter.precio} Bs
                    </p>
                  </div>
                </div>
                <div className="">
                  {select?.inverterId == inverter._id ? (
                    <button
                      onClick={() => {
                    ...
                    (select?.inverterId == "" || select?.inverterId == undefined) &&
                    <button
                      onClick={() => {
                        dispatch(
                          addToCart({
                            ...
                            inverterMarca: inverter.marca,
                            inverterModelo: inverter.modelo,
                            inverterImgURL: inverter.imgURL,
                            inverterPotenciaNominalW: inverter.potenciaNominalW,
                            inverterPotenciaPicoW: inverter.potenciaPicoW,
                            inverterPrecio: inverter.precio,
                          })
                        );
                        toast.success("Inversor seleccionado");
                      }}
